...
use Kir\CanonicalAddresses\PhoneNumbers\PhoneNumber;
...
return function ($phoneNumber) {
	...
	$startsWith = function (array $list, $length) use ($phoneNumber) {
		$needle = substr($phoneNumber, 0, $length);
		return in_array($needle, $list);
	};

	$split = function ($str, $pos) {
		return array(substr($str, 0, $pos), substr($str, $pos));
	};

	$areaCode = null;

	switch (true) {
		case $startsWith($service2, 2): list($areaCode, $phoneNumber) = $split($phoneNumber, 2); break;
		case $startsWith($service3, 3): list($areaCode, $phoneNumber) = $split($phoneNumber, 3); break;
		case $startsWith($service4, 4): list($areaCode, $phoneNumber) = $split($phoneNumber, 4); break;
		case $startsWith($mobile_4digits, 4): list($areaCode, $phoneNumber) = $split($phoneNumber, 4); break;
		case $startsWith($mobile_3digits, 3): list($areaCode, $phoneNumber) = $split($phoneNumber, 3); break;
		case $startsWith($ndcs4, 4): list($areaCode, $phoneNumber) = $split($phoneNumber, 4); break;
		case $startsWith($ndcs3, 3): list($areaCode, $phoneNumber) = $split($phoneNumber, 3); break;
